...
class YouTubeDownloader:
    ...
    def start_download(self):
        ...
        self.current_download = threading.Thread(target=self.download_playlist, args=(url,))
    ...
    def download_playlist(self, url):
        ...
        try:
            ...
            ydl_opts = {
                ...
                'ignoreerrors': True,
                'no_warnings': False,
                'extractaudio': output_format == 'mp3',
                'audioformat': 'mp3' if output_format == 'mp3' else None,
            ...
            self.log_message(f"Download path: {self.download_path}")
            
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                # Extract playlist info first
                self.log_message("Extracting playlist information...")
                info = ydl.extract_info(url, download=False)
                
                if info.get('_type') == 'playlist':
                    total_videos = len(info['entries'])
                    self.log_message(f"Found playlist: {info.get('title', 'Unknown')}")
                    self.log_message(f"Total videos: {total_videos}")
                else:
                    self.log_message("Single video detected")
    ...
    def progress_hook(self, d):
        ...
        if d['status'] == 'downloading':
            ...
            self.progress_label.config(text=f"Downloading... {percent} at {speed}, ETA: {eta}")
            self.log_message(f"Downloading: {d.get('filename', 'Unknown')} - {percent}")
            ...
            self.log_message(f"Download completed: {d.get('filename', 'Unknown')}")
    ...
    def on_closing(self):
        """Handle window closing"""
        if self.is_downloading:
            if not messagebox.askokcancel("Quit", "Download in progress. Quit anyway?"):
                return
        self.save_config()
        self.root.destroy()
