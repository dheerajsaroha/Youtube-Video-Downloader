#!/usr/bin/env python3
"""
Enhanced YouTube Playlist Downloader with Individual Video Progress
A modern GUI application for downloading YouTube playlists with individual video tracking
"""
...
class YouTubeDownloaderEnhanced:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Enhanced YouTube Playlist Downloader")
    ...
    def setup_gui(self):
        ...
        self.download_all_btn.grid(row=0, column=4, padx=(20, 5))
        
        # Videos List Section
        videos_frame = ttk.LabelFrame(main_frame, text="Videos", padding="5")
        videos_frame.grid(row=3, column=0, columnspan=3, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 10))
        ...
        column_configs = {
            ...
            'actions': {'heading': 'Actions', 'width': 120}
        }
        ...
        overall_frame.grid(row=4, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=(0, 10))
    ...
    def analyze_playlist(self):
        """Analyze playlist and populate video list"""
        ...
        try:
            ...
            messagebox.showerror("Error", f"Failed to analyze playlist: {str(e)}")
            self.log_message(f"Error analyzing playlist: {str(e)}")
